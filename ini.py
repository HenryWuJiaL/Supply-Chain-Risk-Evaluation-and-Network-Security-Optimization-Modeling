...
def simulate_np(tcap, tef):
    # Protection
    ld_p = PlotConfig('RS P - Local Discovery: Secure Configuration', 0, 10, 30, 0.9)  # Unprivileged local discovery
    ca_p_sc = PlotConfig('RS P - Credential Access: Secure Configuration', 40, 65, 85, 0.7)  # OS Dump
    ca_p_pam = PlotConfig('RS P - Credential Access: Privileged Account Management', 50, 75, 85, 0.9)  # OS Dump
    ...
    rs_ca_p_complement_v = np.concatenate([np.array_split(ca_p_sc.v, 2)[0], np.array_split(ca_p_pam.v, 2)[0]])
    np.random.default_rng(seed=get_next_seed()).shuffle(rs_ca_p_complement_v)
    rs_ca_p_complement = PlotConfig('P RS - Credential Access: Secure Configuration\nand Privileged Account Management', vector=rs_ca_p_complement_v)

    # RD: Test account access on different Azure accounts. Describe that it is not necessary to simulate, as there is no real detection or protection
    plot_histogram([ld_p, rs_ca_p_complement], 'Resistance Strength (RS)', 'Density', 'uc_np_discovery_p')

    # Detection
    ld_d = PlotConfig('RS D - Local Discovery: Monitoring', 10, 30, 50, 0.9, color=COLOR_DETECTION)
    ca_d = PlotConfig('RS D - Credential Access: Host IDS', 30, 50, 85, 0.75, color=COLOR_DETECTION)  # Host IDS

    plot_histogram([ld_d, ca_d], 'Resistance Strength (RS)', 'Density', 'uc_np_discovery_d')

    # Remote Service: Cloud Service
    rs_rscs_p_pam = PlotConfig('RS P - RSCS: Privileged Account Management', 50, 80, 90, 0.9)
    rs_rscs_p_mfa = PlotConfig('RS P - RSCS: Multi-factor Authentication', 75, 90, 95, 0.55)

    plot_histogram([rs_rscs_p_pam, rs_rscs_p_mfa], 'Resistance Strength (RS)', 'Density', 'uc_np_eors_p')

    # Account Activity Audit Policies
    rs_rscs_d_aaap = PlotConfig('RS D - RSCS: Account Activity Audit Policies', 20, 60, 80, 1.0, color=COLOR_DETECTION)

    plot_histogram([rs_rscs_d_aaap], 'Resistance Strength (RS)', 'Density', 'uc_np_eors_d')
    ...
    false_p_count = 0
    for i in range(len(tcap.v)):
        if tcap.v[i] < max([ld_d.v[i], ca_d.v[i], rs_rscs_d_aaap.v[i]]):
            # Attack gets detected -> Vuln does not increase
            if false_d_count <= 3:
                print('D: ' + get_printable_table_row([tcap.v[i], ld_d.v[i], ca_d.v[i], rs_rscs_d_aaap.v[i], 1]))
                false_d_count += 1
            continue
        else:
            if true_d_count <= 3:
        ...
        if tcap.v[i] > max([ld_p.v[i], rs_ca_p_complement.v[i], rs_rscs_p_pam.v[i], rs_rscs_p_mfa.v[i]]):
            attacker_wins += 1
            if true_d_count <= 3:
                print('P: ' + get_printable_table_row([tcap.v[i], ld_p.v[i], rs_ca_p_complement.v[i], rs_rscs_p_pam.v[i], rs_rscs_p_mfa.v[i], 0]))
                true_p_count += 1
        else:
            if false_p_count <= 3:
                print('P: ' + get_printable_table_row([tcap.v[i], ld_p.v[i], rs_ca_p_complement.v[i], rs_rscs_p_pam.v[i], rs_rscs_p_mfa.v[i], 0]))
